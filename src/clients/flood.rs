...
impl Flood {
    ...
    async fn login(&self) -> Result<(), Error> {
        let resp = self.client.post(&format!("{}/api/auth/authenticate", &self.url))
            ...
            .send()
            .await?;
        // Log only the status. `{:?}` on a Response includes its headers, and the
        // login response carries Flood's session cookie -- logging it wrote a
        // usable session token to stdout and syslog on every startup.
    ...
    pub(crate) async fn add_torrent_and_start(
        ...
        loop {
            ...
            if !resp.status().is_success() {
                ...
                error!("Could not add torrent, Flood returned HTTP {status}");
                return Err(Error::msg(format!("Can not add torrent (HTTP {status})")));
            }
            response_text = resp.text().await?;
            if response_text.contains("Unauthorized") {
                self.login().await?;
...
// Shape of Flood's /auth/authenticate response:
//   {"success":true,"username":"someuser","level":10}
...
mod test{
    ...
    // Integration test: points at a Flood instance on the author's LAN. Run
    // explicitly with `cargo test -- --ignored` after editing the address.
    #[ignore = "requires a reachable Flood server"]
    #[tokio::test]
