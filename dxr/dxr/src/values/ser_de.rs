...
pub(crate) mod value {
    ...
    impl<'de> Visitor<'de> for ValueVisitor {
            ...
            A: serde::de::MapAccess<'de>,
        {
            ...
            // A `<value>` holds exactly one typed child, so only the first entry
            // carries anything -- but the map still has to be driven to its end
            // before returning.
            //
            // Returning early left the reader parked inside the unconsumed
            // `</value>`. quick-xml 0.30 resynchronised from there; 0.31 onwards
            // does not, and the parent's sequence loop then stopped after the
            // first element: a two-member struct deserialised to one member, a
            // fault lost its faultString, and d.multicall2 returned only the
            // first torrent. In a debug build quick-xml's own debug_assert trips
            // first and every response parse panics instead.
            if let Some(key) = map.next_key()? {
                ...
                // Drive the map to its end so the deserializer is left
                // positioned past `</value>` for whoever reads next. Anything
                // beyond the first entry is not valid XML-RPC, so it is
                // discarded rather than reported.
                while map.next_entry::<de::IgnoredAny, de::IgnoredAny>()?.is_some() {}

                parsed
            } else {
                // `<value></value>` or `<value/>`. next_key() already reported
                // the end of the map, so draining here would read past it and
                // fail with MissingEndTag.
