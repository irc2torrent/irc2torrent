...
fn to_struct_empty() {
    ...
    // quick-xml 0.31 stopped collapsing an empty element into a self-closing
    // tag. `<struct></struct>` is the same XML to any parser -- from_struct_empty
    // below still reads the `<struct/>` spelling.
