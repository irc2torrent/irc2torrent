...
#[quickcheck]
fn to_from_string(string: String) -> TestResult {
    if !super::is_xml_representable(&string) {
...
fn from_to_string(string: String) -> TestResult {
    ...
    // so the string needs to be trimmed and XML-escaped first.
    //
    // partial_escape, not escape: this property compares XML *text*, and the two
    // must agree on which characters get entities. Element text only requires
    // `<`, `>` and `&` escaped, which is what the serializer emits, so escaping
    // `"` here as well made the property compare `&quot;` against a literal
    // quote and fail on a value that had round-tripped perfectly.
    let string = quick_xml::escape::partial_escape(string.trim()).to_string();
